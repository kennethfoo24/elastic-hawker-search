...
- `lib/queries.ts` holds the 4 query builders — the heart of the demo — plus `buildQuery(tier, q, area)`, a single dispatch point from a `TierKey` to its builder shared by the API route and `scripts/validate-chips.ts` (previously each reimplemented the same switch independently). All use the retriever framework; `match` on `semantic_text` is the recommended semantic query. `size: 5` everywhere (top-5 per column).
